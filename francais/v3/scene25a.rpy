# TODO: Translation updated at 2022-04-11 22:36

# game/v3/scene25a.rpy:65
translate francais v3s25a_phoneContinue_d172984c:

    # u "(I should reply to Riley.)"
    u "(Je devrais répondre à Riley.)"

# game/v3/scene25a.rpy:74
translate francais v3s25a_phoneContinue_aaea6a4e:

    # u "(Good for her. She really wanted that job and she'll be awesome at it. Having a school newspaper sounds kind of fun, too.)"
    u "(C'est bien pour elle. Elle voulait vraiment ce poste et elle y sera géniale. Avoir un journal à l'école a l'air assez sympa, aussi.)"

# game/v3/scene25a.rpy:92
translate francais v3s25a_phoneContinue_5bcc2069:

    # u "(What now?)"
    u "(Quoi encore ?)"

# game/v3/scene25a.rpy:115
translate francais v3s25a_phoneContinue_3f50cabf:

    # au "Hey, [name]. Did you guys have a power outage last night in the dorms?"
    au "Salut, [name]. Vous avez eu une coupure de courant la nuit dernière dans les dortoirs ?"

# game/v3/scene25a.rpy:120
translate francais v3s25a_phoneContinue_622d6440:

    # u "Um, no, not that I noticed."
    u "Hum, non, pas que je sache."

# game/v3/scene25a.rpy:125
translate francais v3s25a_phoneContinue_fb83b0a7:

    # au "Hm, weird. Apparently, it's been happening a lot this week."
    au "Hum, c'est bizarre. Apparemment, il y en a beaucoup eu cette semaine."

# game/v3/scene25a.rpy:130
translate francais v3s25a_phoneContinue_29edf0d3:

    # u "Well, if your freezer defrosts and you need to eat all the food before it spoils, let me know."
    u "Si ton congélateur dégèle et que tu dois manger tout ce qu'il y a dedans avant que ça se gâte, fais-moi signe."

# game/v3/scene25a.rpy:135
translate francais v3s25a_phoneContinue_5eeb547f:

    # au "Haha, I think it's mainly full of ice cream, so... the Chicks will definitely have that covered."
    au "Haha, je crois qu'il est surtout plein de crème glacée, alors... les Chicks vont s'en occuper sans problème."

# game/v3/scene25a.rpy:140
translate francais v3s25a_phoneContinue_ddc0c8b2:

    # u "Haha, fair enough."
    u "Haha, j'imagine."

# game/v3/scene25a.rpy:145
translate francais v3s25a_phoneContinue_20f9ed5d:

    # au "Anyway, I'm telling people it might be a good idea to stock up on candles or flashlights."
    au "En gros, je dis à tout l'monde que ça pourrait être une bonne idée de faire des réserves de bougies ou de lampes de poche."

# game/v3/scene25a.rpy:150
translate francais v3s25a_phoneContinue_e32be105:

    # u "Shit. SVC is going back to the stone age."
    u "Oh merde. Le SVC va retourner à l'âge de pierre."

# game/v3/scene25a.rpy:155
translate francais v3s25a_phoneContinue_a03be3c8:

    # au "Yeah, exactly. *Laughs*"
    au "Ouais, on dirait. *Rire*"

# game/v3/scene25a.rpy:160
translate francais v3s25a_phoneContinue_517ca4c0:

    # u "Thanks for the heads up."
    u "Merci du conseil."

# game/v3/scene25a.rpy:165
translate francais v3s25a_phoneContinue_37f493e0:

    # au "No problem."
    au "Aucun problème."

# game/v3/scene25a.rpy:173
translate francais v3s25a_phoneContinue_fdcba548:

    # au "Also..."
    au "Sinon..."

# game/v3/scene25a.rpy:175
translate francais v3s25a_phoneContinue_c09a9208:

    # au "I'm free for a date tonight."
    au "Je suis dispo pour un rencard ce soir."

# game/v3/scene25a.rpy:180
translate francais v3s25a_phoneContinue_504642fa:

    # u "Oh. Are you?"
    u "Oh. C'est vrai ?"

# game/v3/scene25a.rpy:185
translate francais v3s25a_phoneContinue_9391fa0f:

    # au "Yes. I thought that might be useful information for a certain gentleman. *Giggles*"
    au "Oui. Je m'suis dit que cette information pourrait intéresser un certain gentleman. *Ricane*"

# game/v3/scene25a.rpy:190
translate francais v3s25a_phoneContinue_21406404:

    # u "Hmm, actually..."
    u "Hum, à vrai dire..."

# game/v3/scene25a.rpy:192
translate francais v3s25a_phoneContinue_e9c6d785:

    # u "I think I know a certain gentleman who would be very interested to hear this information."
    u "Je crois que je connais un certain gentleman qui est très intéressé d'avoir eu cette information."

# game/v3/scene25a.rpy:197
translate francais v3s25a_phoneContinue_ca2ac7ee:

    # au "*Laughs* Let's do something. Something real."
    au "*Rire* Viens on va faire un truc. Un truc sérieux."

# game/v3/scene25a.rpy:202
translate francais v3s25a_phoneContinue_324fdcfe:

    # u "You're serious? Like a serious date? You're ready?"
    u "T'es sérieuse ? Genre un vrai rencard ? T'es prête ?"

# game/v3/scene25a.rpy:207
translate francais v3s25a_phoneContinue_c3f85570:

    # au "Quit asking before I change my mind, haha."
    au "Arrête de me poser cette question avant que je change d'avis, haha."

# game/v3/scene25a.rpy:212
translate francais v3s25a_phoneContinue_dc08b74a:

    # u "Okay, deal. Dinner tonight. I'll sort the details, stay by the phone."
    u "Ok, ça marche. Un dîner ce soir. Je m'occupe de tout et toi, tu restes à côté de ton téléphone."

# game/v3/scene25a.rpy:217
translate francais v3s25a_phoneContinue_5886be9a:

    # au "Ooh, sounds perfect."
    au "Ooh, ça m'a l'air parfait."

# game/v3/scene25a.rpy:222
translate francais v3s25a_phoneContinue_3e28e4ad:

    # u "Great, see you later."
    u "Super, alors à plus tard."

# game/v3/scene25a.rpy:240
translate francais v3s25a_phoneContinue_7620de10:

    # u "Let's get this day started, shall we?"
    u "Allez ! Que cette journée commence !"
# TODO: Translation updated at 2022-05-14 15:02

# game/v3/scene25a.rpy:135
translate francais v3s25a_phoneContinue_af3db20d:

    # au "I think it's mainly full of ice cream, so... the Chicks will definitely have that covered."
    au "Je crois qu'il est surtout plein de crème glacée, alors... les Chicks vont s'en occuper sans problème."

# game/v3/scene25a.rpy:140
translate francais v3s25a_phoneContinue_c027cabe:

    # u "Ha, fair enough."
    u "J'imagine."

# game/v3/scene25a.rpy:155
translate francais v3s25a_phoneContinue_3a8e89fb:

    # au "Yeah, exactly."
    au "Ouais, on dirait."

# game/v3/scene25a.rpy:185
translate francais v3s25a_phoneContinue_af0ab57f:

    # au "Yes. I thought that might be useful information for a certain gentleman."
    au "Oui. Je m'suis dit que cette information pourrait intéresser un certain gentleman."

# game/v3/scene25a.rpy:197
translate francais v3s25a_phoneContinue_00536740:

    # au "Let's do something. Something real."
    au "Viens on va faire un truc. Un truc sérieux."

# game/v3/scene25a.rpy:207
translate francais v3s25a_phoneContinue_a280e2bd:

    # au "Quit asking before I change my mind!"
    au "Arrête de me poser cette question avant que je change d'avis."

# game/v3/scene25a.rpy:212
translate francais v3s25a_phoneContinue_708b293d:

    # u "Okay, deal. Dinner tonight. I'll sort the details. Stay by the phone."
    u "Ok, ça marche. Un dîner ce soir. Je m'occupe de tout et toi, tu restes à côté de ton téléphone."
